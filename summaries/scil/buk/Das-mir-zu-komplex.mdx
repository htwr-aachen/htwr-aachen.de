...
Hier nehmen wir einfach mal das CLIQUE Problem, also ob ein vollständig verbundener Teilgraph bestimmter Größe existiert. Wir versuchen also CLIQUE $\le_p$ VERTEX_COVER zu zeigen.

<Spoiler>

Wir haben eine Instanz von Clique heißt eine Graphen $G=(V,E)$ und eine Zahl $k \in \mathbb{N}$.

Nun nehmen wir unsere Funktion $f$, $f((G,k))= (G', k')$ mit $G'=(V, \{(v, w) \mid v, w \in V; v\neq w; (v, w) \not\in E\})$ und $k'= |V| - k$.
...
Warum hilft uns dass?... Remember wir müssen unsere korrekte CLIQUE Instanz auf eine korrekte VERTEX COVER Instanz schicken... Wir finden auf unserem Komplementgraphen $G'$ jetzt eine VERTEX Cover, also ein Knotenmenge, wo alle Kanten mindestens ein Ende mit einem Knoten aus $D$ verbunden ist. Das heißt alle Knoten, die nicht in $D$ sind, haben im neuen Graphen keine Kanten untereinander, aber dann natürlich im ursprünglichen. $V\D$ ist in $G$ also eine Clique mit mindestens $|V|-k$ Knoten.

Auch hier ist wieder wichtig zu sagen, dass das offensichtlich polynomiell ist.

Und die Korrektheit zu zeigen (maybe in der Klausur noch mehr begründen):
