...
export const FacultiesNavLink = (props: FacultiesNavLinkProps) => {
	...
	return (
		<li
			className={clsx(
				"w-full border-t border-dotted border-border",
				props.autoBorder ? "last: border-b" : "border-b",
			)}
		>
			<Link
				data-tooltip-id={id}
				...
				href={props.href}
				passHref
				className="m-0 grid size-full grid-cols-[auto_1fr_auto] items-center px-4 py-2 text-sm font-medium hover:border-b-0  hover:bg-primary/15 lg:px-0 no-b"
			>
				<div className="ml-2 block">
					<InternalLinkIcon className="mr-2 inline size-[13px]" />
					{props.children}
					{props.subElement && (
						<span className="block font-sans text-sm font-light lg:mb-3">
