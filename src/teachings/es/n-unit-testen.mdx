...
<Spoiler>
  ...
  Dann würden die Knoten, 1, 2 *trivialerweise*, dann 3, da $i=0 < 1$, dann 4, dann 5, da $j=1 < 2$, dann 6, da $1 < 2$, dann 7, 8, 9.  
</Spoiler>

x> Geben sie nun einen Pfadüberdeckungstest an, oder erläutern sie warum es keinen geben kann.

<Spoiler>
  Einen Pfadüberdeckungstest ist, wegen der doppelten Schleife nicht praktikable bzw. unmöglich, da sei eine Unendliche Pfad-Menge haben. 
  Mit jedoch dem Bounded-Iterior Pfadtest mit einmaligem Durchlaufen ist ebenfalls das Eingabearray
  ```java
  int[] a = {2,1}; 
  ```
  ein minimales.
