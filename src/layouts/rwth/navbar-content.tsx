...
import { LinkElement, NavbarConfig } from "@/models/layout";
  ...
  return (
    <div className="px-6 lg:hidden">
      <ul className="block w-full">
        {config.linkElements.map((x, i) => {
          return (
            <li key={x.path + x.name + i}>
              {x.links && x.links.length > 0 ? (
                <>
                  ...
                  <ul className="pl-6">
                    {x.links.map((subLink, subIndex) => (
                      <li key={subLink.path + subLink.name + subIndex}>
                        <Link
                          href={subLink.href}
                          scroll={false}
